...
# 三个智能体
# 输入是此时观察到的 观察空间的维度 3*(18,)
# 智能体根据相应的 观测空间输入 转换为相应的动作 3*(5,)
class Agents:
    def __init__(self, n_obs, n_action):
        ...
        agent1.load_state_dict(torch.load( '../Assignment2/agents/random_network/agent1.pth' ))

        agent2 = Actor(n_obs, n_action)
        agent2.load_state_dict(torch.load( '../Assignment2/agents/random_network/agent2.pth' ))

        agent3 = Actor(n_obs, n_action)
        agent3.load_state_dict(torch.load( '../Assignment2/agents/random_network/agent3.pth' ))
    ...
    def act(self, obs):
        ...
        for i in range(N_AGENT):
            ...
            # 转换为 one-hot 向量， 例如 action=3 -> [0,0,0,1,0]
